...
stopped" dialog. It's harmless to Hydrogen's own `Adom-Workspace`, but it looks to the user
like Hydrogen broke their Docker — terrible first-run UX. So every Hydrogen operation must be
...
### code-server is an Hydrogen-OWNED CHILD process
code-server (`CODE_SERVER_CHILD` in `runtime/wsl.rs`) runs as a child of Hydrogen, deliberately
tied to Hydrogen's lifetime — **NOT a detached `nohup`** (a detached process dies when WSL
tears down the session anyway). Consequence: every Hydrogen restart (rebuild / crash /
relaunch) orphans the old code-server, so a fresh Hydrogen can come up with the distro
`Running` but the editor DEAD ("Starting AI Environment" forever) even when setup is
otherwise complete.

Fix (already in Hydrogen): Hydrogen **re-ensures code-server on every launch**
...
The Hydrogen control API is on loopback **`http://127.0.0.1:<dynamic>`** (NOT 9001). The
port is dynamic per launch — read the live URL from `~/.adom/hydrogen-control-url`. From
...
| `GET /container-stats` | Live CPU/RAM/disk from the `workspace_stats()` probe (see hydrogen-container-stats) |
...
- **Cause**: code-server child was orphaned by an Hydrogen restart, or the distro went
  unreachable after sleep.
- **Check**: `[wsl] Workspace unhealthy …` in the Hydrogen log (the real signal). NOTE: a
...
- **Cause**: code-server (Hydrogen's child) isn't up even though `wsl -l -v` shows Running.
- **Fix**: relaunch Hydrogen (it re-ensures code-server on launch), or Restart Workspace.
...
- **Fix**: the "WSL2 Not Available" floaty → "Run Setup Steps" walks the user through
  enabling it. See [hydrogen-workspace-monitoring](../hydrogen-workspace-monitoring/SKILL.md).
...
3. **If Running but editor dead**: code-server child orphaned → Restart Workspace
   (`wsl --terminate Adom-Workspace` + re-ensure code-server), or relaunch Hydrogen.
...
- [hydrogen-workspace-monitoring](../hydrogen-workspace-monitoring/SKILL.md) — the 15s poll, floaty states, lifecycle dialog, auto-start
- [hydrogen-container-stats](../hydrogen-container-stats/SKILL.md) — title-bar CPU/RAM bars + `workspace_stats()` probe
- [hydrogen-networking](../hydrogen-networking/SKILL.md) — port 7380 code-server, control port 47084, host reachability
