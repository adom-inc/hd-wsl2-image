...
description: >
  ...
  hydrogen-bootstrap set). Start here to find the right skill for any Hydrogen topic.
  These skills are runtime-neutral and apply on every host Hydrogen runs on
  (Windows, macOS, Ubuntu); platform layers add their own *-windows (etc.)
  companions. Trigger words: Hydrogen skills, hydrogen, what skills, find
  skill, Hydrogen help, Hydrogen overview, skill index, Hydrogen reference.
---

# Hydrogen — Skill Index

Hydrogen (ah) is Adom's standalone Tauri v2 app that runs the full
Hydrogen experience locally. One exe manages your workspace, VS Code,
Claude Code, the bridges, and the Adom Wiki — zero cloud dependency. Hydrogen runs
on Windows, macOS, and Ubuntu.

This index lists the **platform-generic** Hydrogen skills (the `hydrogen-bootstrap` set).
...
## The generic Hydrogen skills (hydrogen-bootstrap)

| Skill | What it covers |
|---|---|
| **hydrogen-adom-auth** | Adom login/logout, session token, profile menu, how auth feeds setup. |
| **hydrogen-adom-desktop** | What Hydrogen and Adom Bridge (ab) are and how they work together — ab is your hands on the user's real OS (files, shell, OS screenshots, app launch, bridges). |
...
| **hydrogen-bridges** | Built-in KiCad/Fusion/Puppeteer bridges + extensible wiki catalog (e.g. Blender) + ab capability list. |
...
| **hydrogen-desktop-sse** | Local workspace API + SSE — how adom-cli inside the workspace controls Hydrogen (replaces Carbon locally). |
| **hydrogen-eda-discovery** | EDA tool discovery / setup flow. |
| **hydrogen-eval** | Inject JS into any Hydrogen surface (shell/workbench/wiki/Claude) via /eval-in — add-ons, overlays, highlights, drive UI. |
| **hydrogen-instapcb** | InstaPCB integration in Hydrogen. |
...
| **hydrogen-open-url** | The decision guide for "open a website" — the ways (webview tab, Hydrogen window, Pup, native browser, picker, headless curl) and when to pick each. |
| **hydrogen-overview** | This index. |
| **hydrogen-permissions** | Why mic/camera/clipboard/notifications "just work" in Hydrogen — webview permissions are auto-granted, so webview apps see no Allow prompts. |
| **hydrogen-profile-menu** | The avatar dropdown: profile/repos/molecules links and logout. |
| **hydrogen-self-screenshot** | How the AI screenshots Hydrogen panels + any host window (window control, multi-monitor), injects to shotlog, runs visual self-checks. |
| **hydrogen-self-update** | How Hydrogen updates itself. |
| **hydrogen-settings** | Hydrogen's Settings dialog + the full preferences tree (theme, vim mode, 3D control style, launch-on-login, ab window behavior). |
...
| **hydrogen-ui** | Hydrogen's UI: menus, dialogs, panels, CDP-driven navigation, brand/CSS conventions. |
