...
description: >
  What Hydrogen (ah) and Adom Bridge (ab) are, what each does, and how
  they work together — so the AI in the workspace knows it has HANDS on the
  user's real operating system. Hydrogen is the workspace app the user looks at; ab is
  ...
  native), and drive KiCad / Fusion 360. Hydrogen bundles ab during install and runs
  it in embedded mode, but ab is a standalone product that ALSO connects Adom
  cloud containers to a user's machine. Hydrogen also has a BUILT-IN desktop bridge of
  its own that serves a subset of ab's verbs.
  READ THIS to understand your reach beyond the workspace and how to invoke it.
  Trigger words — adom desktop, ab, ad cli, what can you do on my computer, reach
  ...
  embedded mode, ab hidden, hd_status, hd_api, shell_execute, adom-bridge-cli ping,
  adom-bridge-cli status.
---

# Hydrogen ↔ Adom Bridge — your reach onto the user's real machine

You (the AI) run inside the **Hydrogen workspace** (a local Linux workspace on the
user's machine). That's a sandbox. **Adom Bridge (ab) is how you reach OUT of it
onto the user's actual operating system** — their real files, shell, screen, and
desktop apps. Knowing ab exists is the difference between "I can only work in
here" and "I can create that file on your Desktop, screenshot your screen, and
open it in KiCad."

> **Platform note:** Hydrogen's host layer (file/shell/screenshot/notification/window
...
| What the user sees | The window they work in. | Usually nothing — Hydrogen hides it (embedded mode). |
...
| Runs without the other? | Hydrogen spawns ab. | **Yes** — ab is a standalone product; it also connects Adom *cloud* containers to a user's machine. |

**Why two apps:** ab came first and is genuinely powerful — it's what lets any
Adom cloud container reach into a user's desktop. Rather than reinvent it, Hydrogen
**bundles ab and makes it part of Hydrogen's install**, then puts it in *embedded mode*
so the user only ever manages one app (ah). The two talk to each other
constantly. See [hydrogen-embedded-ad](../hydrogen-embedded-ad/SKILL.md) for the embedding
mechanics (tray ownership, lifecycle, standalone↔embedded transitions).

## Hydrogen's built-in bridge vs the embedded ab (dual-client)
...
1. **Hydrogen's own built-in bridge** — Hydrogen's `hydrogen-app` crate implements a desktop bridge
   *itself* (`commands.rs::handle_command` → `screenshot.rs::handle_desktop_command`).
   It serves a **subset** of ab's verbs directly inside Hydrogen's process:
...
2. **The standalone ab, spawned `--embedded`** by Hydrogen's supervisor
   (`ad_supervisor.rs`). ab serves the **full** verb surface — including ones
   Hydrogen's built-in bridge does NOT implement:
...
"unknown desktop command"), the embedded ab isn't connected — only Hydrogen's built-in
bridge is — so recording / multi-monitor / glob-pull aren't available until ab
is up. Screenshots, window control, files, shell, captions, notifications and
the KiCad/Fusion/browser bridges all work off Hydrogen's built-in bridge regardless.

## What ab/Hydrogen let you do on the user's real machine
...
|---|---|---|
| **Create files/folders on the host** | `send_files` (container → desktop) | [hydrogen-file-transfer](../hydrogen-file-transfer/SKILL.md) |
| **Pull files from the host** | `pull_file` (desktop → container, SHA256-verified); browse + watch via `list_files` / `watch_files` | [hydrogen-file-transfer](../hydrogen-file-transfer/SKILL.md) |
...
| **Record the screen / a tab** | `desktop_record_*` (whole screen), `browser_record_*` (one tab) | [hydrogen-recording](../hydrogen-recording/SKILL.md) — ab-served, verify `record` capability |
...
| **Embedded mechanics (tray/lifecycle)** | `desktop_embedded_*` | [hydrogen-embedded-ad](../hydrogen-embedded-ad/SKILL.md) — ab-served |
...
shell). In **embedded mode Hydrogen auto-approves** shell commands so the AI isn't
...
runners** over hand-quoted `shell_execute` (ab ≥ 1.8.50):
...
If KiCad or Node.js is missing on the host, Hydrogen can trigger an unattended
...
## Talking to Hydrogen + ab from the CLI
...
**Hydrogen control (the Hydrogen app itself):**
```bash
adom-bridge-cli hd_status            # is Hydrogen running + composed runtime state
adom-bridge-cli hd_screenshot        # window-bounded Hydrogen shot (resize <1500px before reading!)
adom-bridge-cli hd_log               # tail Hydrogen's log
adom-bridge-cli hd_launch            # start Hydrogen detached
# Direct Hydrogen control API (HTTP, no shell-escaping pain):
adom-bridge-cli hd_api '{"method":"POST","path":"/setup/run-all","body":{}}'
adom-bridge-cli hd_api '{"method":"GET","path":"/health"}'
```
`hd_api` hits Hydrogen's **control API** on its discovered port (fallback **47084**).
See the [[hydrogen-adom-desktop-windows]] companion for the host port-discovery file.

**Embedded-mode introspection (ab-served):**
```bash
...
You (AI in the workspace)
  └─ adom-bridge-cli <verb> '<json>'        ← CLI in the workspace
       └─ relay  (adom-bridge-cli serve: WS :8765 + HTTP :8766, started in the workspace)
            └─ WebSocket ──► Hydrogen's built-in bridge AND/OR the embedded ab on the user's machine
                 └─ acts on the OS / KiCad / Fusion / Chrome
```

- In Hydrogen the relay is started for you by the **`start-relay`** setup step; Hydrogen's
  built-in bridge and the embedded ab both connect to it. The host-side clients
...
- **Never kill the host ab process** — the host ab is your relay bridge;
...
- **Never hardcode `:9001` as Hydrogen's control API.** `9001` is real — it's ab's
  liveness beacon that ab probes to detect Hydrogen — but Hydrogen's actual control API is
...
my machine" — that's an **ab** job, and you can do it. When it's about the
workspace itself (editor, files under `/home/adom/project`, the runtime), that's
**Hydrogen**. Lead with what you can do, verify with `adom-bridge-cli ping`, then act.

## Related skills
- [[hydrogen-adom-desktop-windows]] — Windows/WSL2 host specifics (paths, winget installs, loopback forwarding, taskkill)
- [hydrogen-embedded-ad](../hydrogen-embedded-ad/SKILL.md) — how Hydrogen bundles + embeds ab (tray, lifecycle, standalone↔embedded)
- [hydrogen-bridges](../hydrogen-bridges/SKILL.md) — the KiCad / Fusion / Puppeteer bridge ecosystem
- [hydrogen-file-transfer](../hydrogen-file-transfer/SKILL.md) — send/pull files, browse + watch the host filesystem
- [hydrogen-self-screenshot](../hydrogen-self-screenshot/SKILL.md) — host screenshots + window control
- [hydrogen-recording](../hydrogen-recording/SKILL.md) — screen vs tab recording (ab-served)
...
- [hydrogen-runtime-mode](../hydrogen-runtime-mode/SKILL.md) — the Hydrogen↔ab relationship is identical across runtimes
