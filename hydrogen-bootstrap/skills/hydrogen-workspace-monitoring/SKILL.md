...
[[hydrogen-workspace-monitoring-windows]].
...
- [[hydrogen-workspace-monitoring-windows]] — the WSL2 "Not Available" floaty, `wsl --terminate` stop, the 7380 reachability check, the no-daemon framing
- `hydrogen-workspace-lifecycle` — provision/export/remove/stop, the never-touch-global rule, control endpoints
- `hydrogen-container-stats` — the title-bar CPU/RAM bars and `workspace_stats()` probe
- `hydrogen-networking` — code-server port, control port
