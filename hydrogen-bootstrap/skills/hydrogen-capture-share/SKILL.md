...
description: >
  The in-Hydrogen screen-share + AI-consent surface that lets YOU (the AI) screenshot
  ...
  ("AI wants access" + your reason), the ~3s auto-approve in Hydrogen, and the
  cancelable CountdownToast before capture starts. READ THIS before trying to
  screenshot the editor — a screenshot of Hydrogen's own UI requires an active share.
...
This is the surface **you (the AI) trigger to see Hydrogen's own screen.** To screenshot
the *editor* (panels, the workspace, or the whole monitor) into the container, a
**screen share must be active first.** You request it, the user consents once (Hydrogen
...
> ab with zero dialogs. This skill is the **in-webview** share that captures what
> Hydrogen itself is rendering (the editor) via the browser's `getDisplayMedia`/Element
...
|---|---|---|
| **Share this tab** | "Panels and workspace screenshots" | Captures Hydrogen's own webview — panels + workspace |
...
capture/recording targets: Hydrogen's own embedded Webview, or a Pup (Puppeteer)
browser window. Webview = "see Hydrogen itself"; Pup = "see the AI-driven browser."
...
| `workspace` | `captureWorkspace()` | the whole Hydrogen workspace surface |
...
When `'RestrictionTarget' in window` and `getDisplayMedia` are available, Hydrogen can
...
> Native (Tauri) note: inside Hydrogen, tab/screen snapshots come from the native CDP
...
When you (or a bridge) need screen/mic access, Hydrogen shows a consent gate before any
...
- The dialog header reads **"AI wants access"** and shows **your `reason`** in a
  ...
  "capture screen.")
- **Hydrogen auto-approves after ~3 s** (`AUTO_MS = 3000`), showing
  "Auto permission will be given in N…". This is the Hydrogen-only fast path so you're
...
- On approve, Hydrogen starts the screen share first (tab or screen), then mic if
...
1. **To screenshot Hydrogen's editor you need an active share.** If you're not sharing,
...
4. **Pick the right scope:** `panel` (one panel, Element-Capture-restricted),
   `workspace` (the Hydrogen surface), or `screen` (full display). Use Window mode
   (Webview vs Pup) to choose Hydrogen's own UI vs the AI browser.
5. **For host-OS screenshots that need no share at all,** use ab's
   `desktop_screenshot_*` instead (see Related skills).

## Related skills
- [hydrogen-recording](../hydrogen-recording/SKILL.md) — screen/mic recording (same share + countdown machinery; in-app .webm recorder + ab desktop recording)
...
- [hydrogen-adom-desktop](../hydrogen-adom-desktop/SKILL.md) — ab's dialog-free host OS screenshots (`desktop_screenshot_screen/_window`)
