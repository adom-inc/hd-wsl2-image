...
- See `hydrogen-setup` skill for details
...
> The Bridges section / "Bridge Manager" item was removed (it was a no-op with no backing panel). For the bridge panel itself, see `hydrogen-bridges`.
...
See `hydrogen-networking` skill for port details.
...
3. **Not in the list?** That absence is the signal: the element isn't wired to the bus yet. It
   ...
   hydrogen-desktop repo — see that repo's `hydrogen-ui-actions` skill). Don't fall back to clicking;
