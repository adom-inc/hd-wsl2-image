...
sharing. See `hydrogen-capture-share` for the sharing/approval UX.
...
`GET $CTRL/ui/actions` for the live set of menus/dialogs you can open (see `hydrogen-ui`).
...
repeat. Log shots for the user with `shotlog inject -c hydrogen-ui -d "<desc>" <path>`
