...
ps(){ local why=$1 script=$2; $AB run_script "$(python3 -c 'import json,sys,base64;print(json.dumps({"scriptB64":base64.b64encode(sys.argv[1].encode("utf-16-le")).decode(),"shell":"powershell","reason":sys.argv[2],"timeoutSeconds":600}))' "$script" "$why")" 2>&1; }
