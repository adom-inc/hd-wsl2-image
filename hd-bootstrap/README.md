...
```
adompkg install adom/hd-bootstrap
```
...
`adom/hd-bootstrap` depends on `adom/core` and adds only what's specific to
...
adom/core                ← the Adom ecosystem (shared with web Hydrogen)
  └─ adom/hd-bootstrap    ← THIS: HD-runtime skills + daemon + HD editor config
```
...
(plus `adom/hd-bootstrap` only if their capability needs HD's runtime), and
...
> `ADOMPKG_ALLOW_SUDO=1 adompkg install adom/hd-bootstrap`.
...
`hd-wsl2-image/image/bake-hd-setup.sh`:
...
- ✅ **Skills bundled into `skills/`** — the **31 platform-generic** `hd-*` skills.
  The other 15 are Windows-specific (in `adom/hd-windows-bootstrap`); 13 mixed
...
- **InstaPCB:** the generic bundle **includes `hd-instapcb`** (and `hd-who-am-i`'s
...
**PUBLISHED + PUBLIC** at `adom/hd-bootstrap@0.2.0` on wiki.adom.inc. Deps
...
(`bake-via-bootstrap.sh`) installs `adom/hd-windows-bootstrap`, which pulls this
