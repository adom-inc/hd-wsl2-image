...
description: >
  ...
  hd-capture-share). HD's webview engine is platform-specific; the per-engine
  implementation detail (e.g. WebView2 on Windows) lives in the platform companion (see
  [[hd-permissions-windows]]). Trigger words — permission, permissions, webview permission,
...
Every request is unconditionally allowed. In the HD log you'll see lines like `[perm]
auto-granted permission kind …` and, at startup, `[perm] Registered PermissionRequested
auto-grant handler`.

> **Platform note:** HD uses a different webview engine per platform (e.g. WebView2 on
...
> Windows/WebView2 implementation and its code evidence are in [[hd-permissions-windows]].

## Why HD does this

HD is the user's **trusted desktop app** — they've already chosen to run it. Intermediate
...
surfaces are HD's own panels and the user's own webview apps. So HD grants them all up
front.
...
- If a webview app *isn't* getting mic/camera data, the cause is **not** a denied
  permission prompt (HD never denies). Look elsewhere: no device present, the page served
  over a non-secure context, a code bug, or the OS-level privacy toggle for the camera/mic
  being off for the app entirely.
- This is behavior baked into HD's webview host — it's not something the web page or the
  user configures.
...
- [[hd-permissions-windows]] — the WebView2 (Windows) implementation + lib.rs evidence
