...
> Platform layers add a `*-windows` companion ([[hd-container-stats-windows]]) with the host-specific probe — the WSL `workspace_stats()` health check, `wsl --terminate` restart, the benign `[docker]` log line, and distro disk semantics.
...
When `restart needed` shows, open the Adom menu → Restart Workspace (see the `[[hd-ui]]` skill for the Adom menu). A "Restart" here terminates and re-ensures the workspace + code-server.
...
The authoritative "which runtime am I on" log line is `[app] Active workspace runtime: …`. (Platform-specific log signals — e.g. the benign Windows `[docker]` heartbeat — are covered in `[[hd-container-stats-windows]]`.)
...
- `[[hd-workspace-monitoring]]` — the 15s state poll + floaty states behind these badges
- `[[hd-container]]` — workspace context: relay, bridges, env vars
