...
- code-server is alive in the distro but WSL2 localhost-forwarding wedged (a
  ...
  daemon installs a host-side proxy to recover this — see `hd-port-watcher`.
  Or restart HD to re-establish the forward.

## Related skills

- `hd-networking` — the broader picture: `0.0.0.0` auto-forward, hostnames,
  proxy architecture, relay vs direct connect, firewall rules, NEVER do these
- `hd-port-watcher` — the retained daemon that registers `127.0.0.1`-only
